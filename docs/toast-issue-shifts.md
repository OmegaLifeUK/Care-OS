...
### Step 10 — Controller fix wasn't enough
After fixing the controller (removing eager-loading of nullable BelongsTo), the toast STILL appeared. The Laravel log was empty — no server errors at all. The endpoint was returning valid JSON with HTTP 200.

### Step 11 — Replaced `alert()` with `console.error()`
The real issue: FullCalendar's `failure` callback used `alert('Failed to load shifts')` — a blocking browser alert. Even if the failure was intermittent or the endpoint eventually succeeded, the `alert()` was firing and couldn't be dismissed without user interaction.

Replaced both `alert()` calls in `schedule-shift.js` with `console.error()`:
- Line 416: `alert('Failed to load resources')` → `console.error()` with error details
- Line 428: `alert('Failed to load shifts')` → `console.error()` with error details

This way, any genuine failures still get logged to the browser console for debugging, but don't block the user.

## Files Modified
- `app/Http/Controllers/frontEnd/Roster/Staff/CarerController.php` — removed nullable BelongsTo eager-loading, added manual staff/client lookup maps, added try/catch
